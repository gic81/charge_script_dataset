...
if [ "$#" -eq  "0" ]
  then
    ...
    echo "    Usage: ./asmfish_time_test.sh  execution_time_in_minutes"
    echo "    Example: ./asmfish_time_test.sh log.txt 30"
    echo "" 
 else
  re='^[0-9]+$'
  if ! [[ $2 =~ $re ]] ; then
    ...
    echo "    Usage: ./asmfish_time_test.sh namefile.txt  execution_time_in_minutes"
    echo "    Example: ./asmfish_time_test.sh log.txt 30" 
    echo "" >&2; exit 1
  fi 
    time=$(date +%s)
    ...
    while [ $SECONDS -le $end ]; do
       ...
       #echo q | htop -C | aha --line-fix | html2text -width 999 | grep -v "F1Help" | grep -v "xml version=" >> $1
       cpustat 0.333 1 >> $1
        #ps -C asmfish >/dev/null && echo "Running" || echo "Not running"
       ...
       if [ $SECONDS -gt $(($end - 3)) ]; then
          echo "   ***   Asmfish session completed!   ***"
          echo -e "\n   ***   Asmfish session completed!   ***" >> $1
          echo -e "\n - Asmfish has been launched $counter times" >> $1
          time=$(date +%s) 
          echo -e "\n End Time: $time" >> $1 
       fi
